...
library("oce")
library("ggplot2")
all_metrics_ = all_metrics
drops <- c("FOG.NRI", "FOG.PSK", "FOG", "meanSentenceLength", "meanWordSyllables")
all_metrics = all_metrics[,!(names(all_metrics) %in% drops)]
...
complex_met = scale(all_metrics[,5:47])              #Scale variables
...
ggsave(paste(path,"imagens\\number_clusters.png", sep = ""), scale = 3)

library(clValid)
library(kohonen)
library(mclust)
valid_test <- clValid(complex_met, 2:10,
                      clMethods = c("kmeans"),
...
ggsave(paste(path,"imagens\\clusters.png", sep = ""), scale = 3)
