...
#drops <- c("Farr.Jenkins.Paterson")
#cent = centers_unscale[,!(names(centers_unscale) %in% drops)]
#add_column(dataset, d = 4:8, .after = "b")
#cent = centers_unscale[,colnames(all)[5:52]]
#centers_unscale = cent
...
rownames(data)=c("max", "min", "1", "2", "3", "4", "5")
...
plot(evaluate_metrics$NCSKEW_mean, evaluate_metrics$DUVOL_mean,cex = evaluate_metrics$size_clusters/14, xlim = c(-0.3,0.15), ylim = c(-0.17,0))#
text(evaluate_metrics$NCSKEW_mean, evaluate_metrics$DUVOL_mean,label=c("1", "2", "3", "4", "5"))
dev.off()

png(paste(path,"ncskew_vs_crashcount.png"), units = "px", width=2500, height=1700, res=200)
plot(evaluate_metrics$NCSKEW_mean, evaluate_metrics$CRASH_COUNT_mean,cex = evaluate_metrics$size_clusters/14, xlim = c(-0.3,0.15), ylim = c(-1,0.15))#
text(evaluate_metrics$NCSKEW_mean, evaluate_metrics$CRASH_COUNT_mean,label=c("1", "2", "3", "4", "5"))
dev.off()

png(paste(path,"crashcount_vs_duvol.png"), units = "px", width=2900, height=1800, res=200)
plot(evaluate_metrics$DUVOL_mean, evaluate_metrics$CRASH_COUNT_mean,cex = evaluate_metrics$size_clusters/14,xlim = c(-0.17,0), ylim = c(-1,0.15))# 
text(evaluate_metrics$DUVOL_mean, evaluate_metrics$CRASH_COUNT_mean,label=c("1", "2", "3", "4", "5"))
dev.off()
